...
description: "Conventions, authentication, permissions, route table, envelopes, errors, idempotency and example flows."
---
...
The machine-readable contract is [`api/openapi.yaml`](/fleetplane/api-reference/) (OpenAPI 3.0.3, hand-authored, enforced by a contract test — see [ADR-011](/fleetplane/developers/adr/adr-011-openapi/)). This guide is the human companion. For the resource model behind the API, see the [API and resource model design doc](https://github.com/samishal1998/fleetplane/blob/main/docs/04_API_AND_RESOURCE_MODEL.md).

:::tip[Generated reference]
            Every operation in `api/openapi.yaml` also has its own generated page — parameters, request and response schemas — under the [OpenAPI reference](/fleetplane/api-reference/). This guide is the conceptual companion: auth, errors, idempotency, and when a verb answers `200` versus `202`.
            :::
...
- **Colon verbs** — actions that are not plain CRUD are spelled as `POST /v1/<collection>/{id}:<verb>` (`:drain`, `:undrain`, `:park`, `:start`, `:protect`, `:unprotect`, `:reconcile`, `:pause`, `:resume`, `:resolve`). The path is split at the *last* colon; IDs never contain one ([ADR-005](/fleetplane/developers/adr/adr-005-http-router/)). An unknown verb returns 404 `not_found`.
- **IDs** — every entity ID is a prefixed ULID: `res_`, `pool_`, `acq_`, `lease_`, `op_`, `evt_` ([ADR-004](/fleetplane/developers/adr/adr-004-ids/)).
...
The read endpoints beyond the original design (acquisition get, pool reads, operation list, providers, `:resolve`) are documented in [ADR-API-001](/fleetplane/developers/adr/adr-api-001-read-endpoints/). The acquisition list, the pool lifecycle verbs, and the resource verbs that close the write-side gaps are documented in [ADR-API-002](/fleetplane/developers/adr/adr-api-002-operation-completeness/).
...
and `deleteAfter` for [parked machines](/fleetplane/guides/concepts/#parked-machines-the-warm-tier).
...
a deficit reappears ([design doc 05 §6](https://github.com/samishal1998/fleetplane/blob/main/docs/05_RECONCILIATION_AND_SCHEDULING.md)),
...
- `status.phase` is one of `unknown`, `provisioning`, `ready`, `allocated`, `parking`, `parked`, `starting`, `draining`, `deleting`, `failed`, `orphaned`. There is no `deleted` phase — deletion terminality is the storage tombstone, surfaced as `metadata.deletedAt` ([ADR-017](/fleetplane/developers/adr/adr-017-operation-states/)).
- `status.parkedAt` (RFC 3339) appears while the machine is in the parked tier — when it entered `parked` ([concepts → parked machines](/fleetplane/guides/concepts/#parked-machines-the-warm-tier)); it clears on the return to `ready`.
...
Operation states: `journaled` → `in_flight` → `external_accepted` → `verifying` → `uncertain` (frozen for manual `:resolve`); terminal states are `succeeded`, `failed`, `aborted` ([ADR-017](/fleetplane/developers/adr/adr-017-operation-states/)). `GET /v1/operations` lists open (non-terminal) operations.
...
- `POST /admin/backup` with body `{"to": "/abs/path.db"}` — online SQLite backup via `VACUUM INTO`; see the [backup and restore runbook](/fleetplane/guides/operations/backup/)
...
- [`api/openapi.yaml`](/fleetplane/api-reference/) — the machine-readable contract
- [API and resource model](https://github.com/samishal1998/fleetplane/blob/main/docs/04_API_AND_RESOURCE_MODEL.md) design doc — the model behind the routes
- [Security and operations](https://github.com/samishal1998/fleetplane/blob/main/docs/07_SECURITY_AND_OPERATIONS.md) design doc — permissions, ops listener, readiness philosophy
- [ADR-008](/fleetplane/developers/adr/adr-008-tokens/) — token format · [ADR-005](/fleetplane/developers/adr/adr-005-http-router/) — colon verbs · [ADR-011](/fleetplane/developers/adr/adr-011-openapi/) — OpenAPI contract testing · [ADR-017](/fleetplane/developers/adr/adr-017-operation-states/) — operation states · [ADR-API-001](/fleetplane/developers/adr/adr-api-001-read-endpoints/) — additive read endpoints · [ADR-API-002](/fleetplane/developers/adr/adr-api-002-operation-completeness/) — completing the operation surface
