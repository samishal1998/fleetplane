---
title: "Concepts"
description: "Resources, phases, classes, pools, acquisitions and leases, parked machines, the operation journal, ownership and protection."
---
...
For full depth, see the [design docs](/fleetplane/guides/introduction/) (especially [02 Architecture](https://github.com/samishal1998/fleetplane/blob/main/docs/02_ARCHITECTURE.md), [04 API and resource model](https://github.com/samishal1998/fleetplane/blob/main/docs/04_API_AND_RESOURCE_MODEL.md), and [05 Reconciliation and scheduling](https://github.com/samishal1998/fleetplane/blob/main/docs/05_RECONCILIATION_AND_SCHEDULING.md)). Everything described here is also visible in the built-in [web dashboard](/fleetplane/guides/dashboard/), served by `fleetplane serve` at `http://<server.addr>/ui/`.
...
All IDs are `<prefix>_<ULID>` ([ADR-004](/fleetplane/developers/adr/adr-004-ids/)).
...
The kernel never interprets kind- or provider-specific fields. Provider-native data rides along untouched in `status.extensions`, so advanced provider features are never erased (design principle: generic core, typed edges — see [02 Architecture §7](https://github.com/samishal1998/fleetplane/blob/main/docs/02_ARCHITECTURE.md)).
...
There is deliberately **no `deleted` phase**. Deletion terminality is a storage tombstone (`deletedAt` set), not a phase — so "is it gone?" has exactly one answer, and a crash between "phase says deleted" and "row removed" can never happen ([ADR-017](/fleetplane/developers/adr/adr-017-operation-states/)).
...
Classes may contain provider-specific spec fields — they are templates, not portability promises ([04 §2](https://github.com/samishal1998/fleetplane/blob/main/docs/04_API_AND_RESOURCE_MODEL.md)). `reclaim.idleAfter` gives class-created resources that belong to no pool an idle lifetime: once a machine has had no lease for that long, Fleetplane drains and deletes it automatically. That is what makes fire-and-forget `acquire` safe — released capacity does not leak money.
...
Leases, resources, and money have **three different lifetimes**. A lease is how long a caller needs capacity; a resource is the infrastructure providing it; the billing window is what the provider actually charges — often a full hour for a machine that ran ten minutes. Fleetplane keeps the three apart and schedules against the third ([design doc 11](https://github.com/samishal1998/fleetplane/blob/main/docs/11_COST_AWARE_LEASING.md), [ADR-018](/fleetplane/developers/adr/adr-018-cost-aware-leasing/)).
...
Operators can park and start explicitly — `POST /v1/resources/{id}:park` / `:start`, `fleetplane resources park`/`start`, or the dashboard's Park/Start buttons — alongside the automatic policy. The usual gates hold either way: a leased machine is never parked, and parking never changes ownership or identity. Whether the tier is earning its keep is measurable — see the [operations guide's parked-machines metrics](/fleetplane/guides/operations/metrics/#parked-machines-metrics).
...
Fleetplane stamps everything it creates with reserved labels (`fleetplane.io/managed`, `fleetplane.io/owner`, `fleetplane.io/id`, `fleetplane.io/op`, `fleetplane.io/class` — [ADR-013](/fleetplane/developers/adr/adr-013-registration-labels/)). Those labels are the recovery anchor: a periodic **discovery sweep** (`discovery.interval`, default 30s) compares provider reality against local records and resolves three mismatch cases ([ADR-017](/fleetplane/developers/adr/adr-017-operation-states/)):
...
- **Re-adoption**: a labeled provider resource with no local record (e.g. you restored an older database) is re-adopted as `managed` — infrastructure must never become undeletable just because local state was lost ([06 Storage and HA §8](https://github.com/samishal1998/fleetplane/blob/main/docs/06_STORAGE_AND_HA.md)).
...
Protection is a standing flag on a resource, set with `fleetplane resources protect` (or `POST /v1/resources/{id}:protect`) and cleared with `unprotect`. It is the flag the deletion, parking, exclusive-scheduling and idle-reclaim gates already read: a protected resource refuses all four until it is explicitly unprotected. It shows up as `metadata.protected` on the resource envelope, and it is metadata rather than desired state, so setting it does not bump `metadata.generation`.

An operator's explicit `:park` deliberately bypasses the delete-protected gate (protection guards deletion; parking is reversible); the automatic reclaim sweep respects it. See the [HTTP API guide](/fleetplane/reference/http-api/#undrain-protect-unprotect) and the [CLI reference](/fleetplane/reference/cli/#fleetplane-resources-protect--unprotect).
...
Everything above has a visual counterpart in the embedded web dashboard: fleet overview, resources with phase, pools with convergence status, acquisitions, the operation journal (including one-click resolution of `uncertain` operations), events, and provider health. It is served by the same `fleetplane serve` process at `http://<server.addr>/ui/` — no extra deployment. See the [dashboard guide](/fleetplane/guides/dashboard/).

## Further reading

- [02 Architecture](https://github.com/samishal1998/fleetplane/blob/main/docs/02_ARCHITECTURE.md) — layers, registries, desired vs. observed state
- [04 API and resource model](https://github.com/samishal1998/fleetplane/blob/main/docs/04_API_AND_RESOURCE_MODEL.md) — envelopes, classes, pools, leases
- [05 Reconciliation and scheduling](https://github.com/samishal1998/fleetplane/blob/main/docs/05_RECONCILIATION_AND_SCHEDULING.md) — the reconciliation equation, scheduler steps
- [06 Storage and HA](https://github.com/samishal1998/fleetplane/blob/main/docs/06_STORAGE_AND_HA.md) — the journal protocol, SQLite, the HA path
- [11 Cost-aware leasing](https://github.com/samishal1998/fleetplane/blob/main/docs/11_COST_AWARE_LEASING.md) — billing windows, queueing, termination policy
- [12 Parked machines](https://github.com/samishal1998/fleetplane/blob/main/docs/12_PARKED_MACHINES.md) — the stop/resume warm tier
- [ADR-014](/fleetplane/developers/adr/adr-014-retries/) — why the operation engine is the only retry authority
- [ADR-017](/fleetplane/developers/adr/adr-017-operation-states/) — operation states, tombstone deletion, ghost vs. orphan
- [ADR-018](/fleetplane/developers/adr/adr-018-cost-aware-leasing/) — the cost-aware leasing implementation decisions
- [ADR-019](/fleetplane/developers/adr/adr-019-parked-machines/) — the parked-machines implementation decisions
