...
description: "Every config.yaml key, its type, default, and validation rules."
---
...
[`schema/config.schema.json`](https://github.com/samishal1998/fleetplane/blob/main/schema/config.schema.json), published at:
...
:::caution[Strict YAML — unknown fields are boot errors]
The config file may only
contain what the running binary actually implements
([ADR-007](/fleetplane/developers/adr/adr-007-config/)). A typo like `serverr:` or a key from
...
defaults per [ADR-014](/fleetplane/developers/adr/adr-014-retries/). The tables below note
...
|---|---|---|---|
| `server.addr` | string | `":8080"` | Main API listen address. Serves `/v1/*`, `/health/*`, and the embedded [web dashboard](/fleetplane/guides/dashboard/) at `/ui/` (`/` redirects there). |
| `server.opsAddr` | string | `"127.0.0.1:9090"` | Ops listener: `/metrics` (Prometheus), `/debug/pprof/*`, and `POST /admin/backup` (see the [backup runbook](/fleetplane/guides/operations/backup/)). Keep it loopback-only unless the network is trusted. |
...
([ADR-003](/fleetplane/developers/adr/adr-003-sqlite-driver/)); hot backups run via
...
([ADR-007](/fleetplane/developers/adr/adr-007-config/)); values are resolved at boot and never
...
authority ([ADR-014](/fleetplane/developers/adr/adr-014-retries/)).
...
suite ([ADR-020](/fleetplane/developers/adr/adr-020-docker-provider/)). All settings optional.
...
[providers guide](/fleetplane/guides/providers/docker/).
...
| `instanceProfile` | string | — | IAM instance profile **name** attached to created instances; the credential then also needs `iam:PassRole` (see the [setup guide](/fleetplane/guides/providers/aws/#credentials-and-permissions)). |
...
([ADR-013](/fleetplane/developers/adr/adr-013-registration-labels/)). Minimal service-account
permissions are in the
[setup guide](/fleetplane/guides/providers/gcp/#credentials-and-permissions).
...
[04 §2](https://github.com/samishal1998/fleetplane/blob/main/docs/04_API_AND_RESOURCE_MODEL.md)). A resource created "from class
...
([above](#driver-fake)). See the [providers guide](/fleetplane/guides/providers/overview/) for the
per-driver caveats.
...
ghosts and orphans ([ADR-017](/fleetplane/developers/adr/adr-017-operation-states/)). The
...
This mirrors [`examples/config.yaml`](https://github.com/samishal1998/fleetplane/blob/main/examples/config.yaml), expanded
...
- [Dashboard guide](/fleetplane/guides/dashboard/) — the embedded web UI served at `/ui/`
- [Backup and restore runbook](/fleetplane/guides/operations/backup/) — uses `server.opsAddr`
- Design docs: [security & operations](https://github.com/samishal1998/fleetplane/blob/main/docs/07_SECURITY_AND_OPERATIONS.md)
  (tokens, secrets, ops listener),
  [API & resource model](https://github.com/samishal1998/fleetplane/blob/main/docs/04_API_AND_RESOURCE_MODEL.md) (classes, kinds),
  [reconciliation & scheduling](https://github.com/samishal1998/fleetplane/blob/main/docs/05_RECONCILIATION_AND_SCHEDULING.md)
  (pools, discovery),
  [cost-aware leasing](https://github.com/samishal1998/fleetplane/blob/main/docs/11_COST_AWARE_LEASING.md) (billing windows, queueing),
  [parked machines](https://github.com/samishal1998/fleetplane/blob/main/docs/12_PARKED_MACHINES.md) (the stop/resume warm tier)
- ADRs: [ADR-007 config & secrets](/fleetplane/developers/adr/adr-007-config/),
  [ADR-008 tokens](/fleetplane/developers/adr/adr-008-tokens/),
  [ADR-014 retries](/fleetplane/developers/adr/adr-014-retries/),
  [ADR-017 operation states, ghosts & orphans](/fleetplane/developers/adr/adr-017-operation-states/),
  [ADR-018 cost-aware leasing](/fleetplane/developers/adr/adr-018-cost-aware-leasing/),
  [ADR-019 parked machines](/fleetplane/developers/adr/adr-019-parked-machines/)
